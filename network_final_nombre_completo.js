            ...
            if (params.nodes.length > 0) {
                ...
                Object.entries(node).forEach(([key, value]) => {
                    if (!['id', 'label', 'image', 'shape', 'font', 'size'].includes(key)) {
                        ...
                        infoHTML += `<p><strong>${key}:</strong> ${formattedValue}</p>`;
                    }
                });
                ...
                infoHTML += `
                    ...
                    <ul>
                        ${connections.map(id => { const connected = nodes.get(id); return `<li>${connected.id || connected.label}</li>`; }).join('')}
                    </ul>
                `;
            ...
            if (params.edges.length > 0) {
                ...
                Object.entries(edge).forEach(([key, value]) => {
                    if (!['from', 'to', 'id', 'label'].includes(key)) {
                        edgeInfo += `<p><strong>${key}:</strong> ${value}</p>`;
                    }
                });
