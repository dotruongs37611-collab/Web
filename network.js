let clickedEdge, fromNode, toNode;
let clickedEdge;
  ...
  try {
    ...
    network.on("click", function (params) {
      ...
      if (params.nodes.length === 0 && params.edges.length === 0) {
        ...
        clearHighlights();
        clickedEdge = edges.get(params.edges[0]);
        ...
        fromNode = nodesMap[clickedEdge.from];
        toNode = nodesMap[clickedEdge.to];
        ...
        edgeFields.forEach(field => {
          if (clickedEdge[field.key]) {
            let value = autoLinkNames(clickedEdge[field.key], nodesMap);
            const urlMatch = value.match(/https?:\/\/[^\s)]+/);
            if (urlMatch) {
              const url = urlMatch[0];
              value = value.replace(` (${url})`, '').replace(url, '').trim();
