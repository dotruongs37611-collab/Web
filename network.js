  ...
  try {
    ...
    network.on("click", function (params) {
      if (params.nodes.length > 0) {
        ...
        let html = "";
        if (node.image) {
          html += `<img src="\${node.image}" alt="\${node.label}" style="max-height:120px; float:left; margin:0 1rem 1rem 0;">`;
        }
        ...
        edges.get().forEach(edge => {
          if (edge.from === node.id || edge.to === node.id) {
            ...
            html += `<li><a href="#" style="color:#66ccff" onclick="focusNode('\${otherId}')">\${other?.label || otherId}</a></li>`;
          }
        });
        html += `</ul>`;
        for (let key in node) {
        ...
        html += `</div>`;
        html += `<p><strong>Between:</strong> <a href="#" style="color:#66ccff" onclick="focusNode('\${edge.from}')">\${fromNode?.label || edge.from}</a> and <a href="#" style="color:#66ccff" onclick="focusNode('\${edge.to}')">\${toNode?.label || edge.to}</a></p>`;
        for (let key in edge) {
    ...
    searchButton.addEventListener('click', () => {
      ...
      if (!query) return;
      const found = data.nodes.find(n => n.label.toLowerCase() === query);
      if (found) {
        focusNode(found.id);
      } else {
